...
import { contentTypeForPointer, storageKey, writeStoredFile } from "@/lib/storage";

export type PayMethod = "PIX" | "BOLETO";

/**
 * Arquiva o comprovante e devolve o pointer para `Payment.proofPath`.
 *
 * Comprovante de pagamento é prova fiscal, não cache: no disco efêmero do
 * container ele desaparecia no redeploy seguinte, deixando a guia marcada como
 * paga sem nada que sustentasse a baixa.
 */
async function writeProof(opts: {
  ...
  const key = storageKey(
    "proofs",
    opts.firmId,
    `${opts.paymentId}.${extSafe(opts.ext)}`,
  );
  return writeStoredFile({
    key,
    body: opts.buffer,
    contentType: contentTypeForPointer(key),
  });
}
...
export async function confirmGuidePayment(opts: {
  ...
  // Arquiva ANTES de marcar como pago. Se o arquivamento falhar, a guia não
  // pode ser dada como baixada — ficaria PAID sem comprovante recuperável.
  let proofPath: string;
  try {
    proofPath = await writeProof({
      firmId: opts.firmId,
    ...
    await writeStoredFile({
      key: storageKey("proofs", opts.firmId, `${payment.id}.meta.txt`),
      body: [
    ...
    return {
      error:
        e instanceof Error
          ? `Falha ao arquivar o comprovante: ${e.message}`
          : "Falha ao arquivar o comprovante.",
      status: 502 as const,
    };
  }
