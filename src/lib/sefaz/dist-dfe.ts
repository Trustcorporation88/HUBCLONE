...
export function distDfeMock(opts: {
  cnpj: string;
  ultNsu: string;
}): DistDfeResult {
  ...
  for (let i = 1; i <= 3; i++) {
    const nsu = padNsu(String(base + i));
    const accessKey = `${dig.slice(0, 8)}352607${dig}${"55123456789012"}${i}`
      .replace(/\D/g, "")
      .padEnd(44, "0")
      .slice(0, 44);
    const isOut = i % 2 === 1;
    const vNF = (1000 * i + 49.9).toFixed(2);
    const xml =
      `<?xml version="1.0" encoding="UTF-8"?>` +
      `<nfeProc xmlns="http://www.portalfiscal.inf.br/nfe" versao="4.00">` +
      `<NFe><infNFe Id="NFe${accessKey}" versao="4.00">` +
      `<ide><dhEmi>${new Date().toISOString()}</dhEmi></ide>` +
      `<emit><CNPJ>${isOut ? dig : "11222333000144"}</CNPJ></emit>` +
      `<dest><CNPJ>${isOut ? "11222333000144" : dig}</CNPJ></dest>` +
    ...
    docs.push({
      ...
      issuerCnpj: isOut ? dig : "11222333000144",
      recipientCnpj: isOut ? "11222333000144" : dig,
      amountCents: Math.round(Number(vNF) * 100),
      issuedAt: new Date(),
    });
  }

  const ult = padNsu(String(base + 3));
  return {
    cStat: "138",
    xMotivo: "Documento localizado (MOCK DistDFe)",
    ultNsu: ult,
    maxNsu: ult,
    docs,
  };
